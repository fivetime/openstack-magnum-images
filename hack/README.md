...
上游"仍在维护"比我们提供的宽：这套云从 **1.36 起步**（平台集群是 1.36，Cilium
还不支持 1.37；1.37 提前构建备用），更早的 minor 不出镜像也不出集群模板。
`discover.sh` 用 `K8S_MIN_MINOR`（默认 `1.36`，手动触发可用 `min_k8s` 覆盖）把
endoflife.date 返回的老 minor 扣掉；`K8S_LIST` 显式指定的版本不受此限。以后再
淘汰一个系列就抬这个值。
