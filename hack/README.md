...
上游"仍在维护"比我们提供的宽：这套云从 **1.37 起步**，更早的 minor 不出
镜像也不出集群模板。`discover.sh` 用 `K8S_MIN_MINOR`（默认 `1.37`，手动触发
可用 `min_k8s` 覆盖）把 endoflife.date 返回的老 minor 扣掉；`K8S_LIST` 显式
指定的版本不受此限。以后再淘汰一个系列就抬这个值。
